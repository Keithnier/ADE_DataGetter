    ...
    <script type="text/javascript">
        ...
        function start (){
            if (isGoing == false){
                ...
                var locations = region.replace(new RegExp("，","gm"), ",").split(",");
                initParam.west_long = locations[0];
                initParam.south_lat = locations[1];
                initParam.east_long = locations[2];
                initParam.north_lat = locations[3];

                var params = JSON.stringify(initParam);

                if ( region == "" || time == "")
                {
                    alert("Please fill in the crawl params!");
                }else if(!(Twitter||Flickr||Tumblr||YouTube)){
                    alert("Please Choose the platform！");
                } else{
                    $.ajax({
                        url:"/DataGetServlet",//要请求的服务器url
                        data:{option:"start",time:time, region:region,Twitter:Twitter,Flickr:Flickr,Tumblr:Tumblr,YouTube:YouTube, crawlerType:"Location", params:params},  //要向服务器发送的数据
                        async:true,   //是否为异步请求
                        cache:false,  //是否缓存结果
                        type:"POST", //请求方式为POST
        ...
        function pause (){
            ...
            if (isGoing == false){
                ...
                $.ajax({
                    ...
                    data:{oTwitter:Twitter,Flickr:Flickr,Tumblr:Tumblr,YouTube:YouTube},
                    async:true,   //是否为异步请求
                    cache:false,  //是否缓存结果
                    type:"POST", //请求方式为POST
                    dataType:"json",   //服务器返回的数据是什么类型
...
<aside >
    <ul class="menu">
        ...
        <li class="active"><img class="icon" src="../image/left/dataIndex.png"><a href="byregion.jsp" onclick="return isStop()">Crawl By Region</a></li>
    ...
    <section id="con">
        <div class="content">
            <div id="params"  style="display: block">
                ...
                <label title="Rectangular area. For example: -180, -90, 180, 90 &#10 Corresponds to the west longitude, south latitude, east longitude, and north latitude">Region:&nbsp;&nbsp;<input id="region" type="text" name="region" >&nbsp;&nbsp;&nbsp;&nbsp;</label>
            </div>
            <label>
                Platform：&nbsp;&nbsp;
                <input id="Twitter"  type="checkbox" value="Twitter"/>&nbsp;&nbsp;Twitter&nbsp;&nbsp;
                <input id="Flickr"  type="checkbox" value="Flickr" />&nbsp;&nbsp;Flickr&nbsp;&nbsp;
                <input id="Tumblr"  type="checkbox" value="Tumblr" disabled="disabled" style="TEXT-DECORATION: line-through" title="Does not support geographic search."/><s>&nbsp;&nbsp;Tumblr&nbsp;&nbsp;</s>
